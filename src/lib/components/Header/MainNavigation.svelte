	...
	<div class="max-w-7xl mx-auto px-4">
		<div class="flex items-center justify-between py-3">
			...
			<div class="flex items-center gap-6">
				<a href="/" class="flex items-center gap-2 text-xl font-bold text-white hover:text-emerald-400 transition-colors">
					<Code2 class="w-6 h-6 text-emerald-400" />
					CodeOut
				</a>
				...
				{#if user}
					<nav class="hidden md:flex items-center gap-1">
						...
						<MenuNavButton href="/challenge">
							<Code2 class="w-4 h-4 mr-1.5" />
							Challenges
						</MenuNavButton>
						<MenuNavButton href="/home/lobby">
							<Trophy class="w-4 h-4 mr-1.5" />
							Lobbies
						</MenuNavButton>
					</nav>
				{/if}
			...
			<div class="flex items-center gap-3">
				{#if user}
					...
					<DropdownMenu>
						<DropdownMenuTrigger>
							<div class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-neutral-900/50 border border-neutral-700 hover:bg-neutral-800/70 transition-colors cursor-pointer">
								<div class="w-7 h-7 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-400 flex items-center justify-center">
									<User class="w-3.5 h-3.5 text-white" />
								</div>
								<div class="hidden sm:block text-sm">
									<div class="font-medium text-neutral-200 text-sm">{getDisplayName()}</div>
